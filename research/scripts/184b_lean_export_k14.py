"""
184b_lean_export_k14.py
======================
Extend the Lean verification to k=14: gamma = 8619/10000 = 0.8619 (certificate was
...
SECOND lake library `CollatzCert14` in the same project.
...
N = 3 ** (K - 1)
Nl = N // 3
G, D = 8714, 10000            # gamma = 0.8714 (certified target 0.87145)
Q = 48
S = 34
...
v = np.load(os.path.join(here, "certificate_k14.npy")).astype(np.float64)
...
for i in range(N):
    ...
    bad += (not ok)
assert bad == 0, bad
print(f"k=14 pre-check OK: {N:,} inequalities at gamma = {G}/{D}")

root = os.path.join(here, "..", "lean", "CollatzCert")
ddir = os.path.join(root, "CollatzCert14")
...
for c in range(nchunks):
    ...
    with open(os.path.join(ddir, f"Data{c}.lean"), "w", newline="\n") as f:
        f.write(f'namespace CollatzCert14\n\ndef s{c} : String := "{payload}"\n\n'
                f'end CollatzCert14\n')

imports = "\n".join(f"import CollatzCert14.Data{c}" for c in range(nchunks))
